...
export const CvModal: React.FC<CvModalProps> = ({ isOpen, onClose, lang }) => {
  ...
  const SectionTitle = ({ icon: Icon, children }: { icon: React.ElementType; children: React.ReactNode }) => (
    <div className="cv-heading-wrap flex items-center gap-2.5 mb-3"><span className="cv-section-icon w-8 h-8 rounded-lg flex items-center justify-center shrink-0"><Icon className="w-4 h-4" /></span><h3 className="cv-heading text-sm sm:text-base font-bold tracking-tight">{children}</h3></div>
  );
  ...
  return <AnimatePresence><div className="cv-overlay fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-6 overflow-y-auto" onClick={onClose}>
    <motion.div initial={{opacity:0,scale:.98,y:10}} animate={{opacity:1,scale:1,y:0}} exit={{opacity:0,scale:.98,y:10}} transition={{duration:.22}} onClick={e=>e.stopPropagation()} className="cv-paper relative w-full max-w-3xl max-h-[92vh] rounded-2xl shadow-2xl overflow-y-auto">
      ...
      <main className="p-5 sm:p-8 space-y-5">
        <section className="cv-hero-card rounded-2xl border p-5 sm:p-6">
          <div className="flex items-center gap-4 sm:gap-5"><div className="cv-avatar w-20 h-20 sm:w-24 sm:h-24 rounded-2xl overflow-hidden border shrink-0"><img src={PERSONAL_INFO.avatar} alt={PERSONAL_INFO.name} className="w-full h-full object-cover"/></div><div className="min-w-0 flex-1"><h1 className="text-xl sm:text-2xl font-bold tracking-tight">{PERSONAL_INFO.name}</h1><p className="text-xs sm:text-sm font-semibold mt-1">{PERSONAL_INFO.title[lang]}</p></div></div>
          <div className="cv-hero-divider my-4" />
          <p className="cv-muted text-[11px] sm:text-xs leading-[1.7] max-w-2xl">{PERSONAL_INFO.bio.philosophy[lang]}</p>
        </section>
