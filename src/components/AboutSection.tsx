...
export const AboutSection: React.FC<AboutProps> = ({ lang }) => {
  const id = lang === 'id';
  const intro = id ? 'Selamat datang di ekosistem developer saya, sebuah infrastruktur yang saya bangun sendiri untuk menyatukan aplikasi web, bot WhatsApp, dan layanan gateway dalam satu platform.' : PERSONAL_INFO.bio.aboutIntro.en;
  const body = id ? 'Dibangun dari awal, ekosistem ini dirancang agar layanan web dan bot dapat terintegrasi dengan mulus, mendukung otomasi, komunikasi real-time, dan pengelolaan layanan yang skalabel melalui arsitektur yang ringan dan responsif.' : PERSONAL_INFO.bio.aboutBody.en;
  return (
