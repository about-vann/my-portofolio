    ...
    <section id="about" className="w-full">
      <motion.div initial={{ opacity: 0, y: 15 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: '-40px' }} transition={{ duration: 0.5, ease: [0.16, 1, 0.3, 1] }} className="w-full rounded-2xl bg-[#15161c] border border-white/[0.07] p-5 sm:p-6 shadow-xl space-y-3.5">
        <div className="flex items-center gap-2.5">
          ...
          <h2 className="text-sm font-semibold text-white tracking-wide">{id ? 'Tentang' : 'About'}</h2>
        </div>
        <div className="space-y-3.5 text-xs sm:text-[13px] leading-relaxed text-zinc-300 font-normal">
          <p>{PERSONAL_INFO.bio.aboutIntro[lang]}</p>
          <p>{PERSONAL_INFO.bio.aboutBody[lang]}</p>
        </div>
      </motion.div>
    </section>
  );
