    ...
    <div className={`min-h-screen ${isLight ? 'bg-[#f0f2f5] text-zinc-900' : 'bg-[#0e0f14] text-zinc-100'} font-['Poppins',sans-serif] selection:bg-black/10 selection:text-zinc-900 transition-colors duration-300 antialiased`}>
      ...
      <main className="portfolio-main mx-auto w-full px-4 pb-12 sm:px-5 lg:max-w-7xl lg:px-8 xl:px-10">
        ...
        <div className="portfolio-desktop hidden lg:grid lg:grid-cols-[minmax(280px,340px)_minmax(0,1fr)] lg:items-start lg:gap-6 xl:gap-8">
          <aside className="portfolio-sidebar lg:sticky lg:top-24 lg:space-y-6"><HeroSection accent={accent} lang={lang} onDownloadCv={() => setIsCvOpen(true)} /><ContactSection accent={accent} lang={lang} /></aside>
          <div className="portfolio-content min-w-0 space-y-6"><AboutSection accent={accent} lang={lang} /><ProjectsSection accent={accent} lang={lang} /><ArticlesSection accent={accent} lang={lang} colorMode={colorMode} /></div>
        </div>
      ...
      <AIChat isOpen={isAiOpen} onClose={() => setIsAiOpen(false)} lang={lang} accent={accent} colorMode={colorMode} />
    </div>
  );
}
